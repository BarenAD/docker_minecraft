...
ENV MC_SERVER_PATH="${VS_PROJECT_PATH}/server"
ENV MC_DATA_PATH="${VS_PROJECT_PATH}/data"
ENV MC_SCRIPTS_PATH="/usr/local/bin"
ENV MC_CRONTAB_PATH="/home/${VS_SERVER_USER}/crontab"

ENV MC_SCREENING_PROJECT_PATH="\/var\/vintage_story"
ENV MC_SCREENING_SERVER_PATH="${VS_SCREENING_PROJECT_PATH}\/server"
ENV MC_SCREENING_DATA_PATH="${VS_SCREENING_PROJECT_PATH}\/data"
...
RUN useradd -m $MC_SERVER_USER && mkdir -p $MC_SERVER_PATH && mkdir -p $MC_DATA_PATH && mkdir -p $MC_SCRIPTS_PATH \
    ...
    && bash mc_crontab && apt clean && rm -rf /tmp/*
